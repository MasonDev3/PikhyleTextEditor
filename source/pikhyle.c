...
char *Java_HL_keywords[] = {
	...
	"interface", "native", "new", "@Override", "package", "private", "protected", 
	"public", "return", "static", "strictfp", "super", "switch", "synchronized",
	"this", "throw", "throws", "transient", "try", "void", "volatile", 
	"while", "true", "false", "null", "boolean|", "byte|", "char|", "short|",
	"String|", "Object|", "List|", "int|", "long|", "float|", "double|", NULL 
};
char *Go_HL_extensions[] = { 
	".go", NULL
};
char *Go_HL_keywords[] = {
...
char *Lox_HL_extensions[] = {
	".lox", NULL
};
char *Lox_HL_keywords[] = {
	"class", "var", "fun", "while", "print", "return", NULL
};
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (PIKHYLE_TAB_STOP - 1) - (rx % PIKHYLE_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (PIKHYLE_TAB_STOP - 1) - (cur_rx % PIKHYLE_TAB_STOP);
    cur_rx++;
    if (cur_rx > rx)
      return cx;
  }
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++)
    if (row->chars[j] == '\t')
      tabs++;
  free(row->render);
  row->render = malloc(row->size + tabs * (PIKHYLE_TAB_STOP - 1) + 1);
  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % PIKHYLE_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        char welcome[80];
        int welcomelen =
            snprintf(welcome, sizeof(welcome), "pikhylEdit -- version %s",
                     PIKHYLE_VERSION);
...
void editorProcessKeypress() {
  static int quit_times = PIKHYLE_QUIT_TIMES;
  int c = editorReadKey();
